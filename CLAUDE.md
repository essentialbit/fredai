...
- `causal_attribution.py` — deterministic (no LLM call) catalyst ranking for symbols `cascade_engine.detect_major_moves()` flags as a major mover — earnings/FOMC-macro calendar, insider Form 4, news sentiment shift, correlated-neighbor contagion, ranked by weight, empty result is a valid "no clear catalyst" (see FSI L4 note below)
...
**Causal Attribution (FSI L4, shipped)**: `causal_attribution.py` — `attribute_move(symbol, event_type, magnitude, quotes)` ranks candidate catalysts from sources FredAI already ingests, closing the gap where `cascade_engine.detect_major_moves()` flagged *which* symbols moved but nothing ranked *why*, leaving chat to fall back on vague prompt-stuffed narrative (same fabrication risk class already rejected for the #31 chat-hallucination fix). Deterministic, no LLM call: `_earnings_catalyst`/`_macro_catalyst` read `memory_store.get_calendar_events_window()` (new — the existing `get_calendar_events()` only looks forward; this adds the backward-looking `[today - days_back, today + days_forward]` window causal work needs), `_insider_catalyst` reads recent signal-coded Form 4s, `_news_catalyst` reads recent ticker headlines gated on `avg sentiment >= 0.25`, `_correlation_catalyst` finds a same-direction correlated neighbor that moved at least as hard (explicitly labeled contagion, not a confirmed catalyst — weighted lower than the named-source catalysts, same distinction `cascade_engine._correlation_neighbors` already draws). Each candidate carries a fixed weight band; candidates are sorted and the top one becomes `summary`; an empty list with an honest "No clear catalyst found in tracked sources." is a valid, correct result, never a fabricated one. Wired into `cascade_engine.detect_major_moves()` (every flagged event now carries an `attribution` block) and `main.py`'s `/api/cascade/<symbol>` (embedded) plus a standalone `/api/attribution/<symbol>` (`login_required`) added so this can become a Fred-chat tool once #106's agentic tool-use chat merges. Originally built only on sources already merged to main at ship time — the original proposal (#145) also named `central_bank_client.py` (#144, FOMC-statement deltas) and `options_data_client.py` (#128, put/call+IV shifts) as catalyst sources, deferred since neither was merged yet. Both have since merged and are now wired in (#525/#526): `_central_bank_catalyst` (macro-wide, fires when `get_latest_central_bank_delta("Fed")`'s sentiment_delta shifts materially within `CENTRAL_BANK_LOOKBACK_DAYS` of the meeting — note it uses local `date.today()` to match `central_bank_client.py`'s own convention, not UTC) and `_options_catalyst` (per-symbol, fires on an ATM IV jump or put/call volume-ratio swing between `get_options_data_prior_pair()`'s two latest snapshots). Verified via `scripts/verify_causal_attribution.py` against a scratch copy of `sentinel.db` (never the live file): no-catalyst fallback, each of the 7 catalyst sources firing independently on synthetic data, and earnings (0.75) correctly outranking news (<=0.6) when both are present for the same symbol.

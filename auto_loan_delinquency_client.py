"""Auto Loan Delinquency Rate (FRED DRALACBS) -- quarterly household
credit-quality macro badge for secured auto debt.

Federal Reserve series for the delinquency rate on auto loans held by all
commercial banks -- the share of auto loan balances 30+ days past due.
Existing household-finance badges cover unsecured revolving credit stress
(Credit Card Delinquency, DRCCLACBS) and the largest secured-debt class
(Mortgage Delinquency, DRSFRMACBS), but neither tracks auto loan
performance -- a distinct secured-debt class with its own dynamics (faster
repossession cycles, subprime-lending exposure, and a tighter link to
used-vehicle price swings than either credit cards or mortgages).
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DRALACBS"
_CACHE_TTL_S = 21600  # DRALACBS updates quarterly; 6h refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[AutoLoanDelinquency] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[AutoLoanDelinquency] fetch error: {e}")
        return None
...
def compute_auto_loan_delinquency() -> dict | None:
...
def get_auto_loan_delinquency(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_auto_loan_delinquency()
        if data:
            _cache["data"] = data

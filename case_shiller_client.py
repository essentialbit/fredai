...
housing-price macro badge.

Every prior housing-category badge (starts, permits) tracks construction
*activity*, not home *prices*. Case-Shiller is the standard institutional
benchmark for home-price appreciation/depreciation and feeds directly into
household-wealth-effect and inflation-persistence (owners' equivalent rent)
read-throughs -- genuinely distinct from the existing housing-activity
badges.
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=CSUSHPINSA"
_CACHE_TTL_S = 3600  # CSUSHPINSA updates monthly; hourly refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[CaseShiller] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[CaseShiller] fetch error: {e}")
        return None
...
def compute_case_shiller() -> dict | None:
    """{"latest": float, "date": str, "change_yoy_pct": float,
    "trend_12m": {...}, "regime": "appreciating"/"depreciating"/"stable"} or
...
def get_case_shiller(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_case_shiller()
        if data:
            _cache["data"] = data

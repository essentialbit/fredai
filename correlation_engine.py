"""Rolling cross-asset correlation matrix — 30-day and 90-day Pearson
correlations between tracked assets' daily returns.
...
def _fetch_daily_closes(symbols: list[str], delay_s: float = 0.2) -> pd.DataFrame:
    ...
    for i, sym in enumerate(symbols):
        history = fetch_history(sym, period="6mo", interval="1d")
        if history:
...
def calculate_rolling_correlation(symbols: list[str]) -> dict[int, list[dict]]:
    ...
    (~30 or ~90 trading days of the fetched 6-month history). A window is
    omitted entirely if there isn't enough overlapping data for any pair.
    """

...
Census Bureau's monthly durable-goods new-orders series is the standard
leading indicator institutional desks watch for capex momentum: new orders
lead production by one to two quarters, making this genuinely distinct from
the already-shipped Industrial Production badge (which reads current
output, not forward intent).
...
sheet/M2, industrial production) -- never curl, see the documented HTTP/2
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DGORDER"
_CACHE_TTL_S = 3600  # DGORDER updates monthly; hourly refetch is plenty
...
def _trend(series: list[float]) -> dict | None:
    ...
    Same shape as copper_gold_ratio.py/industrial_production_client.py's
    _trend()."""
    if len(series) < 8:
        return None
    latest = series[-1]
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[DurableGoods] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[DurableGoods] fetch error: {e}")
        return None
...
def compute_durable_goods_orders() -> dict | None:
...
def get_durable_goods_orders(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_durable_goods_orders()
        if data:
            _cache["data"] = data

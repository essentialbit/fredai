...
Existing-home resales are the largest slice of the U.S. housing market by
unit count. Distinct from the already-shipped housing badges: Housing
Starts/Permits track new-construction supply, and Case-Shiller tracks price
level -- neither surfaces resale transaction volume/turnover, the
demand-side activity signal this badge fills in.
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=EXHOSLUSM495S"
_CACHE_TTL_S = 3600  # EXHOSLUSM495S updates monthly; hourly refetch is plenty
...
def _trend(series: list[float]) -> dict | None:
    ...
    Same shape as trade_balance_client.py/durable_goods_client.py's _trend()."""
    if len(series) < 8:
        return None
    latest = series[-1]
    baseline = series[:-1]
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[ExistingHomeSales] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[ExistingHomeSales] fetch error: {e}")
        return None
...
def compute_existing_home_sales() -> dict | None:
    """{"latest": float (units, SAAR), "date": str, "change_mom": float,
    "trend_12m": {...}, "regime": "rising"/"falling"/"stable"} or None if the
    ...
    return {
        ...
        "date": latest_date,
        "change_mom": round(change_mom, 0),
        "trend_12m": trend_12m,
        "regime": trend_12m["direction"],
    }


def get_existing_home_sales(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_existing_home_sales()
        if data:
            _cache["data"] = data

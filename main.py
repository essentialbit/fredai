...
from memory_store import (
    ...
    insert_analyst_ratings, get_recent_analyst_ratings,
    insert_analyst_consensus, get_latest_analyst_consensus,
)
...
from analyst_data import fetch_analyst_snapshot
...
def api_watchlist():
    ...
    for w in wl:
        ...
        consensus = get_latest_analyst_consensus(w["symbol"])
        if consensus and consensus.get("target_mean"):
            entry["analyst_target_mean"] = consensus["target_mean"]
        result.append(entry)
...
def api_analyst_ratings(ticker):
    """Recent Wall Street analyst upgrades/downgrades + consensus price target
    (FSI L2). Cache-only -- matches /api/insider's pattern, refreshed by the
    daily analyst_ratings job."""
    sym = ticker.upper()
    days = request.args.get("days", 90, type=int)
    return jsonify({
        "ticker": sym,
        "days": days,
        "consensus": get_latest_analyst_consensus(sym),
        "recent_changes": get_recent_analyst_ratings(sym, days=days),
    })
...
if __name__ == "__main__":
    ...
    def job_analyst_ratings_refresh():
        """Refresh Wall Street analyst upgrade/downgrade history + consensus
        price target daily for portfolio + watchlist symbols (FSI L2). US-listed
        only -- yfinance has essentially no analyst coverage for ASX/crypto."""
        try:
            from memory_store import get_conn as _gc
            with _gc() as c:
                port_rows = [r[0] for r in c.execute("SELECT DISTINCT symbol FROM portfolio WHERE shares > 0").fetchall()]
            ...
            if not symbols:
                return
            new_ratings, covered = 0, 0
            for sym in symbols:
                snap = fetch_analyst_snapshot(sym)
                if not snap:
                    continue
                covered += 1
                new_ratings += insert_analyst_ratings(sym, snap["ratings"])
                if snap["consensus"]:
                    insert_analyst_consensus(sym, snap["consensus"])
            print(f"[AnalystData] Ratings refreshed — {covered}/{len(symbols)} symbols covered, {new_ratings} new rating(s)")
        except Exception as e:
    ...
    scheduler.add_job(job_analyst_ratings_refresh, "cron", hour=8, minute=0, id="analyst_ratings")

"""Mortgage Delinquency Rate (FRED DRSFRMACBS) -- quarterly household
credit-quality macro badge for secured mortgage debt.

Federal Reserve series for the delinquency rate on single-family
residential mortgages held by all commercial banks -- the share of
mortgage balances 30+ days past due. Existing household-finance badges
cover revolving unsecured credit stress (Credit Card Delinquency,
DRCCLACBS), debt volume (Consumer Credit Outstanding, TOTALSL), the cost
of taking on new mortgage debt (30-Year Fixed Mortgage Rate), and home
price levels (Case-Shiller). None of those track whether existing
mortgage holders are actually keeping up with their obligations -- a
distinct secured-debt credit-quality signal for the largest household
debt class.
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=DRSFRMACBS"
_CACHE_TTL_S = 21600  # DRSFRMACBS updates quarterly; 6h refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[MortgageDelinquency] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[MortgageDelinquency] fetch error: {e}")
        return None
...
def compute_mortgage_delinquency() -> dict | None:
...
def get_mortgage_delinquency(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_mortgage_delinquency()
        if data:
            _cache["data"] = data

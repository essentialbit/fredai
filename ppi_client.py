"""Producer Price Index Final Demand (FRED PPIFIS) -- upstream wholesale-
inflation leading-indicator macro badge.

PPI measures cost pressure at the producer/wholesale level, one to two
months ahead of where those costs show up (or don't) in consumer-facing
prices -- genuinely distinct from the already-shipped Core PCE badge, which
reads downstream consumer inflation after retailers have already absorbed
or passed through cost changes. The two series can diverge meaningfully
(e.g. producers eating margin compression instead of passing costs on).
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=PPIFIS"
_CACHE_TTL_S = 3600  # PPIFIS updates monthly; hourly refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[PPI] HTTP {r.status_code}")
            return None
...
def get_ppi(force: bool = False) -> dict | None:

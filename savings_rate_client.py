...
A genuinely new household-balance-sheet signal distinct from the
already-shipped Consumer Credit badge: consumer credit tracks the debt
side of household finances, while the savings rate tracks the surplus
side. A falling savings rate alongside rising consumer credit is a
classic late-cycle warning (households drawing down buffers and
borrowing to sustain spending); a rising savings rate is a defensive
posture.
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=PSAVERT"
_CACHE_TTL_S = 3600  # PSAVERT updates monthly; hourly refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[SavingsRate] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[SavingsRate] fetch error: {e}")
        return None
...
def compute_savings_rate() -> dict | None:
    """{"latest": float (percent of disposable income), "date": str,
    "change_mom": float, "trend_12m": {...}, "regime": "rising"/"falling"/
...
def get_savings_rate(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_savings_rate()
        if data:
            _cache["data"] = data

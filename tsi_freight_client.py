...
freight/logistics activity macro badge.

TSIFRGHT is the Bureau of Transportation Statistics' monthly, seasonally
adjusted composite of trucking, rail, air, water, pipeline and warehousing
freight movement volume (index, 2000=100). First domestic freight/logistics
activity gauge shipped -- distinct from the already-shipped Baltic Dry
Index proxy, which tracks ocean dry-bulk spot shipping *rates* rather than
actual US freight movement *volume* across all modes.

Same free fredgraph.csv fetch pattern already used by every other
FRED-sourced badge (industrial production, jobless claims, breakeven
inflation) -- never curl, see the documented HTTP/2 gotcha in project
memory; plain requests.get matches every shipped FRED client's real code
path.
"""
import csv
import io
...
_CSV_URL = "https://fred.stlouisfed.org/graph/fredgraph.csv?id=TSIFRGHT"
_CACHE_TTL_S = 3600  # TSIFRGHT updates monthly; hourly refetch is plenty
...
def _fetch_series() -> list[tuple[str, float]] | None:
    try:
        ...
        if r.status_code != 200:
            print(f"[TSIFreight] HTTP {r.status_code}")
            return None
        rows = list(csv.reader(io.StringIO(r.content.decode("utf-8"))))
    except Exception as e:
        print(f"[TSIFreight] fetch error: {e}")
        return None
...
def compute_tsi_freight() -> dict | None:
...
def get_tsi_freight(force: bool = False) -> dict | None:
    """Cached accessor -- recomputes at most once per _CACHE_TTL_S."""
    now = time.time()
    if force or _cache["data"] is None or now - _cache["computed_at"] > _CACHE_TTL_S:
        data = compute_tsi_freight()
        if data:
            _cache["data"] = data
